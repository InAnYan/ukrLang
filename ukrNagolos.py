...
# 1. Загрузить словарь наголосов как список
# 2. Выбрать 10 случайных наголосов в список
# 3. Начать игру (цикл):
# 3.1. Показать следующее слово и ожидать ответ наголоса
# 3.2. Записать в список из 10 наголосов ответ пользователя
# 3.3. Повторять к 3.1 до тех пор, пока не будет пройдено 10 слов
# 4. Показать результаты (цикл):
# 4.1. Показать слово, реальный наголос и пользовательский наголос
# 4.2. Если правильно, то инкрементировать количество правильных ответов и показать слово "правильно"
# 4.3. Если не правильно, показать слово "неправильно"
# 4.4. Повторять к 4.1. до тех пор, пока не будет пройдено 10 слов
# 4.5. Показать общую статистику
...
ukrDictFile = open('ukrDict1.txt', 'r', encoding='utf-8')
ukrDict = json.load(ukrDictFile)
ukrDictFile.close()
